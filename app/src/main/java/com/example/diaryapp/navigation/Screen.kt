package com.example.diaryapp.navigation

import com.example.diaryapp.utils.Constants.WRITE_SCREEN_ARGUMENT_KEY

sealed class Screen(val route: String) {

    object Authentication : Screen(route = "authentication_screen")
    object Home : Screen(route = "home_screen")

    //Question mark means that the argument that define here will be optional
    //diaryId(Actual Key)={diaryId} {Actual Value}
    object Write :
        Screen(route = "write_screen?$WRITE_SCREEN_ARGUMENT_KEY=" +
                "{$WRITE_SCREEN_ARGUMENT_KEY}") {

        fun passDiaryId(diaryId: String) = "write_screen?$WRITE_SCREEN_ARGUMENT_KEY=$diaryId"
    }
}